...
def create_device_information_service(config):
    ...
    model = config.get(CONF_MODEL, project.get("name", _HOST_DEFAULT_MODEL))
    version = config.get(
...
def final_validate_config(config):
    # max_clients is host-soft (BlueZ allows multiple centrals); just sanity check
    # against the optional esp32_ble max_connections if present.
